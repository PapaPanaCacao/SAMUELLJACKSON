...
function Socket(model){
	...
	this.deserialize = function(s, seq)
	{
    var a = s[0];
...
//      console.log("client.deserialize.DOWN 1");
    ...
    console.log("client.deserialize.growSnake");
        ...
        console.log("client.deserialize.BONUS 2set");
			...
			var snake2Head = getModel().getSnake(1).getHead();
        
        if(snake2Head.equals(getModel().getBonuses()[0]))
				bonusToChange = 0;
...
// Log messages from the server
	this.connection.onmessage = (e)=> {
		console.log('Socket.onmessage Recieved message ');
    ...
    if (ind > 0)
      ind--;
		console.log(" NOW is : " + ((Math.floor( Date.now() / 1000 ))-first));
		//console.log(" timestamp recieved : " + parseInt(array[ind]));
      ...
      console.log("Socket.onmessage.else");
			...
			this.deserialize(array[0],seq);
		}
		
		this.count =0;
		ViewRefresh();
	}
	this.done = ()=>{clearInterval(this.extrap.interval); this.sendMessage("DONE")}
};